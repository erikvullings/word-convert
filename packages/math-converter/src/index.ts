// @ts-expect-error KaTeX does not publish declarations for its DOM-free ESM subpath.
import rawKatex from 'katex/dist/katex.mjs';
import type { Equation } from '@wordconvert/document-model';

interface KatexApi {
  renderToString(
    expression: string,
    options: {
      displayMode: boolean;
      output: 'htmlAndMathml';
      throwOnError: boolean;
      strict: 'error';
      trust: false;
    },
  ): string;
}

const katex = rawKatex as KatexApi;
export const KATEX_STYLES = `@font-face{font-family:KaTeX_Main;src:url(data:font/woff2;base64,) format("woff2");font-display:block}.katex{font:normal 1.21em KaTeX_Main,serif;line-height:1.2;text-indent:0}.katex-display{display:block;margin:1em 0;text-align:center}.katex-html{white-space:nowrap}.katex .katex-mathml{position:absolute;clip:rect(1px,1px,1px,1px);overflow:hidden;width:1px;height:1px}.katex .base{position:relative;display:inline-block}.katex .strut{display:inline-block}.katex .mspace{display:inline-block}.katex .vlist-t{display:inline-table;table-layout:fixed}.katex .vlist-r{display:table-row}.katex .vlist{display:table-cell;position:relative;vertical-align:bottom}.katex .vlist>span{display:block;height:0;position:relative}.katex .frac-line{border-bottom-style:solid;border-bottom-width:.04em;width:100%}.katex .sqrt>.root{margin-left:.2778em;margin-right:-.5556em}.katex .sqrt>.sqrt-sign{position:relative}.katex .array{display:inline-table;vertical-align:.25em}.katex .array>span{display:table-row}.katex .array>span>span{display:table-cell;text-align:center;padding:0 .5em}`;

export type MathOutputMode = 'source' | 'mathml' | 'katex' | 'disabled';

export type NormalizedMath =
  | { type: 'row'; children: NormalizedMath[] }
  | { type: 'identifier' | 'number' | 'operator' | 'text'; value: string }
  | { type: 'fraction'; numerator: NormalizedMath; denominator: NormalizedMath }
  | { type: 'root'; radicand: NormalizedMath; degree?: NormalizedMath }
  | {
      type: 'script';
      base: NormalizedMath;
      subscript?: NormalizedMath;
      superscript?: NormalizedMath;
    }
  | { type: 'matrix'; rows: NormalizedMath[][] }
  | { type: 'unsupported'; name: string; text: string };

export interface MathConversionResult {
  normalized: NormalizedMath;
  tex: string;
  mathml: string;
  complete: boolean;
  unsupported: string[];
  fallbackText: string;
}

export interface FormulaRenderOptions {
  mode: MathOutputMode;
  display: boolean;
  source?: string;
}

export function renderEquation(
  equation: Equation | undefined,
  options: FormulaRenderOptions,
): string {
  if (options.mode === 'disabled') return '';
  if (!equation)
    return '<span class="formula-error" role="math">Equation unavailable</span>';
  if (equation.source.format === 'omml') {
    const converted = convertOmml(equation.source.value);
    return renderFormula(converted.normalized, {
      ...options,
      source: converted.fallbackText,
    });
  }
  if (options.mode === 'source')
    return escapeHtml(equation.tex ?? plainText(equation.source.value));
  if (options.mode === 'mathml') {
    if (equation.source.format === 'mathml')
      return `<span class="formula-source" role="math">${escapeHtml(plainText(equation.source.value))}</span>`;
    return renderTexSafely(
      equation.tex ?? equation.source.value,
      options.display,
    );
  }
  return renderTexSafely(
    equation.tex ?? equation.source.value,
    options.display,
  );
}

function renderTexSafely(tex: string, display: boolean): string {
  try {
    return katex.renderToString(tex, {
      displayMode: display,
      output: 'htmlAndMathml',
      throwOnError: true,
      strict: 'error',
      trust: false,
    });
  } catch {
    return `<span class="formula-error" role="math" aria-label="Formula could not be rendered">${escapeHtml(safeDiagnostic(tex))}</span>`;
  }
}

interface XmlElement {
  name: string;
  children: Array<XmlElement | string>;
}

const containers = new Set([
  'oMath',
  'oMathPara',
  'e',
  'num',
  'den',
  'deg',
  'sub',
  'sup',
]);
const operators = /^[+\-−=<>≤≥×÷±∓∞∫∑∏√∂∇≈≠∈∉∪∩]+$/u;
const numbers = /^\p{N}+(?:[.,]\p{N}+)?$/u;

export function convertOmml(source: string): MathConversionResult {
  let normalized: NormalizedMath;
  try {
    normalized = normalize(parseXml(source));
  } catch {
    normalized = {
      type: 'unsupported',
      name: 'malformed-omml',
      text: plainText(source),
    };
  }
  const unsupported = collectUnsupported(normalized);
  const tex = toTex(normalized);
  const mathmlBody = toMathMl(normalized);
  return {
    normalized,
    tex,
    mathml: `<math xmlns="http://www.w3.org/1998/Math/MathML">${mathmlBody}</math>`,
    complete: unsupported.length === 0,
    unsupported,
    fallbackText: unsupported.length
      ? `Unsupported formula (${unsupported.join(', ')}): ${safeDiagnostic(textOf(normalized))}`
      : safeDiagnostic(textOf(normalized)),
  };
}

export function renderFormula(
  math: NormalizedMath,
  options: FormulaRenderOptions,
): string {
  if (options.mode === 'disabled') return '';
  const accessible = safeDiagnostic(textOf(math)) || 'Formula';
  if (options.mode === 'source') {
    const source = options.source ? plainText(options.source) : accessible;
    return `<span class="formula-source" role="math" aria-label="${escapeAttribute(accessible)}">${escapeHtml(source)}</span>`;
  }
  if (options.mode === 'mathml')
    return `<span class="formula-mathml" role="math" aria-label="${escapeAttribute(accessible)}">${toMathMlDocument(math)}</span>`;
  const tex = toTex(math);
  const rendered = renderTexSafely(tex, options.display);
  return rendered.includes('formula-error')
    ? `<span class="formula-error" role="math" aria-label="Formula could not be rendered">${escapeHtml(accessible)}</span>`
    : rendered;
}

export function toTex(math: NormalizedMath): string {
  switch (math.type) {
    case 'row':
      return math.children.map(toTex).join('');
    case 'identifier':
    case 'number':
    case 'operator':
      return escapeTex(math.value);
    case 'text':
      return `\\text{${escapeTexText(math.value)}}`;
    case 'fraction':
      return `\\frac{${toTex(math.numerator)}}{${toTex(math.denominator)}}`;
    case 'root':
      return math.degree
        ? `\\sqrt[${toTex(math.degree)}]{${toTex(math.radicand)}}`
        : `\\sqrt{${toTex(math.radicand)}}`;
    case 'script':
      return `${toTex(math.base)}${math.subscript ? `_{${toTex(math.subscript)}}` : ''}${math.superscript ? `^{${toTex(math.superscript)}}` : ''}`;
    case 'matrix':
      return `\\begin{matrix}${math.rows.map((row) => row.map(toTex).join(' & ')).join(' \\\\ ')}\\end{matrix}`;
    case 'unsupported':
      return `\\text{${escapeTexText(safeDiagnostic(math.text) || math.name)}}`;
  }
}

export function toMathMlDocument(
  math: NormalizedMath,
  display = false,
): string {
  return `<math xmlns="http://www.w3.org/1998/Math/MathML"${display ? ' display="block"' : ''}>${toMathMl(math)}</math>`;
}

function normalize(node: XmlElement): NormalizedMath {
  const name = localName(node.name);
  if (containers.has(name))
    return row(node.children.filter(isElement).map(normalize));
  if (name === 'r') return token(textContent(node));
  if (name === 'f')
    return {
      type: 'fraction',
      numerator: normalizeChild(node, 'num'),
      denominator: normalizeChild(node, 'den'),
    };
  if (name === 'rad') {
    const degreeNode = child(node, 'deg');
    const degree =
      degreeNode && textContent(degreeNode).trim()
        ? normalize(degreeNode)
        : undefined;
    return {
      type: 'root',
      radicand: normalizeChild(node, 'e'),
      ...(degree ? { degree } : {}),
    };
  }
  if (name === 'sSup' || name === 'sSub' || name === 'sSubSup') {
    const subscript = child(node, 'sub');
    const superscript = child(node, 'sup');
    return {
      type: 'script',
      base: normalizeChild(node, 'e'),
      ...(subscript ? { subscript: normalize(subscript) } : {}),
      ...(superscript ? { superscript: normalize(superscript) } : {}),
    };
  }
  if (name === 'm')
    return {
      type: 'matrix',
      rows: children(node, 'mr').map((matrixRow) =>
        children(matrixRow, 'e').map(normalize),
      ),
    };
  if (name === 'mr') return row(children(node, 'e').map(normalize));
  if (name.endsWith('Pr')) return { type: 'row', children: [] };
  return { type: 'unsupported', name, text: textContent(node) };
}

function token(value: string): NormalizedMath {
  if (numbers.test(value)) return { type: 'number', value };
  if (operators.test(value)) return { type: 'operator', value };
  if (/^[\p{L}\p{Nl}]+$/u.test(value)) return { type: 'identifier', value };
  return { type: 'text', value };
}

function row(children: NormalizedMath[]): NormalizedMath {
  return children.length === 1 ? children[0]! : { type: 'row', children };
}

function normalizeChild(node: XmlElement, name: string): NormalizedMath {
  const found = child(node, name);
  return found
    ? normalize(found)
    : { type: 'unsupported', name: `missing-${name}`, text: '' };
}

function toMathMl(math: NormalizedMath): string {
  switch (math.type) {
    case 'row':
      return `<mrow>${math.children.map(toMathMl).join('')}</mrow>`;
    case 'identifier':
      return `<mi>${escapeHtml(math.value)}</mi>`;
    case 'number':
      return `<mn>${escapeHtml(math.value)}</mn>`;
    case 'operator':
      return `<mo>${escapeHtml(math.value)}</mo>`;
    case 'text':
      return `<mtext>${escapeHtml(math.value)}</mtext>`;
    case 'fraction':
      return `<mfrac>${toMathMl(math.numerator)}${toMathMl(math.denominator)}</mfrac>`;
    case 'root':
      return math.degree
        ? `<mroot>${toMathMl(math.radicand)}${toMathMl(math.degree)}</mroot>`
        : `<msqrt>${toMathMl(math.radicand)}</msqrt>`;
    case 'script':
      return math.subscript && math.superscript
        ? `<msubsup>${toMathMl(math.base)}${toMathMl(math.subscript)}${toMathMl(math.superscript)}</msubsup>`
        : math.subscript
          ? `<msub>${toMathMl(math.base)}${toMathMl(math.subscript)}</msub>`
          : `<msup>${toMathMl(math.base)}${toMathMl(math.superscript ?? { type: 'text', value: '' })}</msup>`;
    case 'matrix':
      return `<mtable>${math.rows.map((r) => `<mtr>${r.map((cell) => `<mtd>${toMathMl(cell)}</mtd>`).join('')}</mtr>`).join('')}</mtable>`;
    case 'unsupported':
      return `<merror><mtext>${escapeHtml(`Unsupported ${math.name}: ${safeDiagnostic(math.text)}`)}</mtext></merror>`;
  }
}

function collectUnsupported(math: NormalizedMath): string[] {
  const names: string[] = [];
  const visit = (value: NormalizedMath): void => {
    if (value.type === 'unsupported') names.push(value.name);
    else if (value.type === 'row') value.children.forEach(visit);
    else if (value.type === 'fraction') {
      visit(value.numerator);
      visit(value.denominator);
    } else if (value.type === 'root') {
      visit(value.radicand);
      if (value.degree) visit(value.degree);
    } else if (value.type === 'script') {
      visit(value.base);
      if (value.subscript) visit(value.subscript);
      if (value.superscript) visit(value.superscript);
    } else if (value.type === 'matrix') value.rows.flat().forEach(visit);
  };
  visit(math);
  return [...new Set(names)];
}

function textOf(math: NormalizedMath): string {
  switch (math.type) {
    case 'identifier':
    case 'number':
    case 'operator':
    case 'text':
      return math.value;
    case 'row':
      return math.children.map(textOf).join('');
    case 'fraction':
      return `${textOf(math.numerator)}/${textOf(math.denominator)}`;
    case 'root':
      return `root ${textOf(math.radicand)}`;
    case 'script':
      return [
        textOf(math.base),
        math.subscript && textOf(math.subscript),
        math.superscript && textOf(math.superscript),
      ]
        .filter(Boolean)
        .join(' ');
    case 'matrix':
      return math.rows.map((r) => r.map(textOf).join(' ')).join('; ');
    case 'unsupported':
      return math.text;
  }
}

function parseXml(source: string): XmlElement {
  if (/<!DOCTYPE|<!ENTITY/i.test(source)) throw new TypeError('Unsafe XML');
  const root: XmlElement = { name: '#document', children: [] };
  const stack = [root];
  const pattern = /<([^>]+)>|([^<]+)/g;
  for (const match of source.matchAll(pattern)) {
    if (match[2]) stack.at(-1)!.children.push(decodeEntities(match[2]));
    else {
      const tag = (match[1] ?? '').trim();
      if (!tag || tag.startsWith('?') || tag.startsWith('!')) continue;
      if (tag.startsWith('/')) {
        if (
          localName(stack.pop()?.name ?? '') !== localName(tag.slice(1).trim())
        )
          throw new TypeError('Mismatched XML');
      } else {
        const selfClosing = tag.endsWith('/');
        const name = tag.replace(/\/$/, '').split(/\s/, 1)[0] ?? '';
        const element: XmlElement = { name, children: [] };
        stack.at(-1)!.children.push(element);
        if (!selfClosing) stack.push(element);
      }
    }
  }
  if (stack.length !== 1) throw new TypeError('Unclosed XML');
  const element = root.children.find(isElement);
  if (!element) throw new TypeError('Missing XML root');
  return element;
}

function isElement(value: XmlElement | string): value is XmlElement {
  return typeof value !== 'string';
}
function localName(value: string): string {
  return value.split(':').at(-1) ?? value;
}
function children(node: XmlElement, name: string): XmlElement[] {
  return node.children
    .filter(isElement)
    .filter((item) => localName(item.name) === name);
}
function child(node: XmlElement, name: string): XmlElement | undefined {
  return children(node, name)[0];
}
function textContent(node: XmlElement): string {
  return node.children
    .map((item) => (isElement(item) ? textContent(item) : item))
    .join('');
}
function decodeEntities(value: string): string {
  return value.replace(
    /&(#x[\da-f]+|#\d+|amp|lt|gt|quot|apos);/gi,
    (_m, entity: string) =>
      entity === 'amp'
        ? '&'
        : entity === 'lt'
          ? '<'
          : entity === 'gt'
            ? '>'
            : entity === 'quot'
              ? '"'
              : entity === 'apos'
                ? "'"
                : String.fromCodePoint(
                    Number.parseInt(
                      entity.slice(entity[1]?.toLowerCase() === 'x' ? 2 : 1),
                      entity[1]?.toLowerCase() === 'x' ? 16 : 10,
                    ),
                  ),
  );
}
function plainText(value: string): string {
  return decodeEntities(value.replace(/<[^>]*>/g, ' '))
    .replace(/\s+/g, ' ')
    .trim();
}
function safeDiagnostic(value: string): string {
  return [...value]
    .filter((character) => {
      const code = character.codePointAt(0) ?? 0;
      return code >= 32 && code !== 127;
    })
    .join('')
    .replace(/[<>]/g, '')
    .slice(0, 500)
    .trim();
}
function escapeHtml(value: string): string {
  return value.replace(/[&<>]/g, (c) =>
    c === '&' ? '&amp;' : c === '<' ? '&lt;' : '&gt;',
  );
}
function escapeAttribute(value: string): string {
  return escapeHtml(value).replace(/"/g, '&quot;').replace(/'/g, '&#39;');
}
function escapeTex(value: string): string {
  return value.replace(/([#$%&_{}])/g, '\\$1').replace(/\\/g, '\\backslash ');
}
function escapeTexText(value: string): string {
  return value
    .replace(/\\/g, '\\textbackslash{}')
    .replace(/([#$%&_{}])/g, '\\$1')
    .replace(/[<>]/g, '');
}
