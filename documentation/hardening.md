...
EPUB content edits are parsed from Markdown into a cloned `DocumentModel` before worker conversion. The editor accepts semantic Markdown constructs, never injects raw HTML into the model, and can retain an existing image only when its generated data URI exactly matches a known document asset. Edited content remains in memory and is discarded when a new source is selected or analysis is rerun.
